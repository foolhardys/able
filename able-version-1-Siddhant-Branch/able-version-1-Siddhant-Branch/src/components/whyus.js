import React from 'react'

const whyus = () => {
  return (
    <>
    <div className=" snap-start font-[poppins] bg-black text-white pb-24 md:h-[100vh] ">
            <div className=" text-center md:w-4xl mx-auto mb-24 ">
                <h2 className="text-5xl font-bold pt-16">WHY US ?</h2>
            </div>
            <div className="justify-center grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            <div className=''>
      <div className='group h-[60vh] w-96'>
        <div className='relative h-full w-full rounded-3xl bg-white dark:bg-[#242424] text-black shadow-xl [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] [backface-visibility:hidden] transition-all duration-1000'>
            <div className='absolute inset-0 text-center mt-32 dark:text-white'>
                <h1>Why us</h1>
            </div>
            <div className='absolute inset-0 h-full w-full rounded-xl px-12 text-center bg-white dark:bg-[#242424] text-black [transform:rotateY(180deg)] [backface-visibility:hidden]'>
                <div className='flex min-h-full flex-col items-center justify-center dark:text-white'>
                    <h1>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Distinctio in placeat nemo esse dicta doloribus ab saepe voluptates obcaecati, impedit delectus. Illum libero, a quae nulla voluptatem sint ab quasi!</h1>
                </div>

            </div>
        </div>
      </div>
    </div>
    <div className=''>
      <div className='group h-[60vh] w-96 '>
        <div className='relative h-full w-full rounded-3xl shadow-xl bg-white dark:bg-[#242424] text-black [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] [backface-visibility:hidden] transition-all duration-1000'>
            <div className='absolute inset-0 text-center mt-32 dark:text-white'>
                <h1>Why us</h1>
            </div>
            <div className='absolute inset-0 h-full w-full rounded-xl px-12 text-center bg-white dark:bg-[#242424] text-black [transform:rotateY(180deg)] [backface-visibility:hidden]'>
                <div className='flex min-h-full flex-col items-center justify-center dark:text-white'>
                    <h1>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eligendi quaerat, temporibus esse autem excepturi quam numquam nemo, vel, necessitatibus voluptate exercitationem quia officia consequuntur doloribus quasi tempora. Consectetur, veniam ducimus?</h1>
                </div>

            </div>
        </div>
      </div>
    </div>
    <div className=''>
      <div className='group h-[60vh] w-96 '>
        <div className='relative h-full w-full rounded-3xl bg-white dark:bg-[#242424] text-black shadow-xl [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] [backface-visibility:hidden] transition-all duration-1000'>
            <div className='absolute inset-0 text-center mt-32 dark:text-white'>
                <h1>Why us</h1>
            </div>
            <div className='absolute inset-0 h-full w-full rounded-xl px-12 text-center bg-white dark:bg-[#242424] text-black [transform:rotateY(180deg)] [backface-visibility:hidden]'>
                <div className='flex min-h-full flex-col items-center justify-center dark:text-white'>
                    <h1>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloremque vitae inventore, ducimus nam, voluptatibus aspernatur expedita debitis ullam quo perspiciatis a quaerat! Fugiat similique maxime harum quia libero nesciunt natus!
                    </h1>
                </div>

            </div>
        </div>
      </div>
    </div>
            </div>
        </div>
        </>
  )
}

export default whyus
